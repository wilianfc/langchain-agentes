"""
LLM as Judge — Avaliação automática de respostas por segmento (loop externo — Cap. 13).

O juiz Claude avalia cada resposta do agente em 4 dimensões calibradas
para o contexto bancário:
  - Fidelidade ao RAG  (a resposta cita apenas o que foi recuperado?)
  - Adequação ao perfil (tom e complexidade adequados ao segmento?)
  - Completude         (cobre todos os pontos da pergunta?)
  - Conformidade LGPD  (sem exposição de dados pessoais?)
...
Uso com Bedrock:
  python llm_judge.py \
    ...
    --resposta "Recomendamos Tesouro Selic com liquidez diária." \
...
_CRITERIOS_BASE = """
1. Fidelidade ao RAG (0-3): a resposta usa apenas informações recuperadas? (0=inventou, 3=totalmente fiel)
2. Adequação ao perfil (0-3): tom e complexidade condizem com o segmento {segmento}? (0=inadequado, 3=perfeito)
3. Completude (0-2): cobre todos os pontos da pergunta? (0=parcial, 2=completo)
4. Conformidade LGPD (0-2): ausência de dados pessoais indevidos? (0=violação, 2=conforme)
"""

_JUDGE_PROMPT = """Você é um avaliador especialista em sistemas de IA para bancos brasileiros.

SEGMENTO DO CLIENTE: {segmento}
CONTEXTO RAG DISPONÍVEL: {contexto}
PERGUNTA DO USUÁRIO: {pergunta}
RESPOSTA DO AGENTE: {resposta}

CRITÉRIOS DE AVALIAÇÃO (pontuação máxima = 10):
{criterios}
...
Responda SOMENTE em JSON com os campos:
  ...
  razao: string (justificativa em até 3 frases)
...
_MOCK_RESPONSES = {
    ...
    "FP": {"fidelidade_rag": 1, "adequacao_perfil": 2, "completude": 2,
           "conformidade_lgpd": 2, "score_total": 7, "aprovado": True,
           "razao": "[MOCK] Fidelidade ao RAG questionável, adequação ao segmento FP OK."},
}
...
class VeredictJudge:
    ...
    def resumo(self) -> str:
        ...
        return (
            ...
            f"  Adequação perfil:  {self.adequacao_perfil}/3\n"
...
def julgar(pergunta: str, resposta: str, contexto: str = "",
    ...
    try:
        ...
        print(f"[AVISO] Bedrock indisponível: {e}", file=sys.stderr)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LLM as Judge — avaliação por segmento")
    parser.add_argument("--modo", choices=["mock", "bedrock", "batch"],
                        default="mock")
    parser.add_argument("--pergunta", help="Pergunta do usuário")
    ...
    if args.modo == "mock":
        ...
        if not args.pergunta or not args.resposta:
            parser.error("--pergunta e --resposta são obrigatórios no modo bedrock")
