"""
Avaliação de métricas de qualidade de resposta (loop externo — Cap. 12).

Métricas implementadas:
  - ROUGE-L  (sobreposição de subsequência mais longa)
  - BERTScore (similaridade semântica via embeddings)
  - G-Eval   (avaliação LLM-based via Bedrock/Claude)
...
class ResultadoAvaliacao:
    ...
    def resumo(self) -> str:
        ...
        if self.geval_score is not None:
            ...
            if self.geval_razao:
                linhas.append(f"  Razão G-Eval: {self.geval_razao}")
        return "\n".join(linhas)


# ── ROUGE-L (sem dependência externa) ────────────────────────────────────────
...
def rouge_l(referencia: str, candidata: str) -> float:
    """Retorna F1 do ROUGE-L entre referência e candidata."""
...
def bertscore_f1(referencia: str, candidata: str) -> Optional[float]:
    """
    Calcula BERTScore F1. Retorna None se bert_score não estiver instalado.
...
_GEVAL_PROMPT = """Você é um avaliador especialista em sistemas de IA para bancos.

Avalie a RESPOSTA GERADA comparando com a RESPOSTA REFERÊNCIA nos critérios:
1. Fidelidade ao contexto (a resposta usa apenas informações disponíveis?)
2. Completude (cobre os pontos essenciais da referência?)
3. Clareza (linguagem adequada para o cliente bancário?)
4. Ausência de alucinações (sem informações inventadas?)
5. Conformidade LGPD (não expõe dados pessoais indevidos?)

Responda SOMENTE em JSON com os campos:
  score: número de 0 a 5 (inteiro)
  razao: string com justificativa em até 2 frases
...
def geval(referencia: str, candidata: str, region: str = "us-east-1",
    ...
    """
    Avaliação G-Eval via Claude no Bedrock.
    Retorna (score, razao) ou (None, None) se Bedrock não disponível.
    """
    try:
        import boto3
        ...
        print(f"[AVISO] G-Eval Bedrock indisponível: {e}", file=sys.stderr)
        return None, None
...
def _demo_offline():
    pares = [
        ("Clientes conservadores priorizam liquidez diária e segurança do capital.",
         "Investidores conservadores preferem produtos com liquidez imediata."),
        ("O Tesouro Direto é garantido pelo governo federal.",
         "O Tesouro Direto oferece garantia do governo."),
        ("Fundos de renda variável possuem alto risco.",
         "Poupança é o investimento mais seguro do mercado."),
    ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Avaliação de métricas de resposta RAG")
    parser.add_argument("--modo", choices=["offline", "geval", "batch"],
                        default="offline", help="Modo de execução")
    parser.add_argument("--referencia", help="Texto de referência (modo geval)")
    ...
    if args.modo == "offline":
        ...
        if not args.referencia or not args.candidata:
            parser.error("--referencia e --candidata são obrigatórios no modo geval")
        ...
        if not args.arquivo:
            parser.error("--arquivo é obrigatório no modo batch")
