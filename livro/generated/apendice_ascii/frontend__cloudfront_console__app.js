(() => {
  ...
  endpointEl.textContent = apiUrl || 'API nao configurada. Ajuste window.APP_CONFIG.apiUrl no deploy.';
  ...
  async function requestJson(path, payload, method = 'POST') {
    if (!apiUrl) {
      throw new Error('API URL nao configurada no frontend.');
    }
  ...
  async function pollStatus(requestId) {
    ...
    while (true) {
      ...
      if (data.status === 'COMPLETED') {
        setBadge(queryBadge, 'success', 'Concluido');

        // result pode chegar como string JSON ou como objeto
        let result = data.result;
        if (typeof result === 'string') {
          try { result = JSON.parse(result); } catch (_) { /* mantem string */ }
        }
  ...
  queryForm.addEventListener('submit', async (event) => {
    ...
    queryStatus.textContent = 'Enfileirando requisicao...';
    ...
    try {
      ...
      if (!accepted.request_id) {
        throw new Error('API nao retornou request_id.');
      }
  ...
  uploadForm.addEventListener('submit', async (event) => {
    ...
    uploadResult.textContent = 'Enviando documento para indexacao...';
  ...
  document.getElementById('fill-document').addEventListener('click', () => {
    ...
    textArea.value = [
      'Clientes conservadores priorizam liquidez, previsibilidade e protecao contra inflacao.',
      'Ofertas recomendadas: Tesouro Selic, CDB com liquidez diaria e fundos DI com baixa taxa.',
      'Perfis de alta renda com horizonte de longo prazo podem receber alocacao gradual em NTN-B e debentures incentivadas.'
    ].join('\n\n');
  });
})();