"""
Golden Dataset — Criação, execução e regressão de qualidade (loop externo — Cap. 14).
...
Fluxo:
  ...
  4. Comparar com baseline anterior e exibir diff de regressão
...
Uso offline (mock):
  ...
  python golden_dataset.py --modo rodar   # executa regressão mock
...
GOLDEN_SAMPLE = [
    {
        ...
        "pergunta": "Qual o melhor investimento para perfil conservador?",
        "resposta_esperada": "Para perfil conservador, recomendamos Tesouro Selic com liquidez diária e CDB com proteção do FGC.",
        "threshold_rouge": 0.35, "threshold_judge": 7,
    },
    {
        "id": "GD-002", "segmento": "PF", "categoria": "cartao",
        "pergunta": "Quais são os benefícios do cartão gold?",
        "resposta_esperada": "O cartão gold oferece programa de pontos, seguro viagem e isenção de anuidade para clientes premium.",
    ...
    },
    {
        ...
        "resposta_esperada": "Empresas podem solicitar capital de giro via conta PJ com análise de faturamento dos últimos 3 meses.",
        "threshold_rouge": 0.30, "threshold_judge": 7,
    },
    {
        "id": "GD-004", "segmento": "FP", "categoria": "investimentos",
        "pergunta": "Quais produtos são indicados para private banking?",
        "resposta_esperada": "Clientes FP têm acesso a fundos exclusivos, COE estruturado e gestão de patrimônio personalizada.",
        "threshold_rouge": 0.30, "threshold_judge": 7,
    },
]
...
_RESPOSTAS_AGENTE_MOCK = {
    "GD-001": "Para clientes conservadores, Tesouro Selic é a opção mais indicada pela liquidez imediata.",
    "GD-002": "O cartão gold dá pontos a cada compra e tem seguro viagem incluído.",
    "GD-003": "Para capital de giro, analisamos o faturamento da empresa nos últimos meses.",
    "GD-004": "Private banking oferece fundos exclusivos e gestão personalizada do patrimônio.",
}
...
def rodar_regressao(caminho_dataset: str = "golden_dataset.jsonl",
    ...
    Para cada caso do golden dataset:
      1. Obtém resposta do agente (mock offline ou real)
    ...
    for caso in casos:
        ...
        if usar_bedrock:
            ...
            resposta_gerada = _RESPOSTAS_AGENTE_MOCK.get(cid, "Resposta não disponível.")
...
def _chamar_agente_bedrock(pergunta: str, segmento: str) -> str:
    """Stub — substituir pelo invoke real do agente LangGraph em produção."""
    raise NotImplementedError("Integração com agente Bedrock não implementada aqui. Use aws_pipeline_clientes.py.")
...
def diff_regressao(baseline: list[dict], atual: list[dict]) -> dict:
    ...
    Compara dois conjuntos de resultados e identifica:
      - Regressões: casos que passaram no baseline e falharam no atual
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Golden Dataset — regressão de qualidade")
    ...
    parser.add_argument("--bedrock", action="store_true",
                        help="Usar Bedrock (padrão: mock offline)")
    ...
    if args.modo == "criar":
        ...
        if not os.path.exists(args.dataset):
            print(f"Dataset não encontrado: {args.dataset}. Criando sample...")
            criar_dataset(args.dataset)
        ...
        if not args.baseline or not args.atual:
            parser.error("--baseline e --atual são obrigatórios no modo diff")
